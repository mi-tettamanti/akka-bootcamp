    ...
    public class GithubWorkerActor : ReceiveActor
    {
        ...
        private void InitialReceives()
        {
            ...
            Receive<RetryableQuery>(query => query.Query is QueryStarrer, query =>
            {
                ...
                var sender = Sender;
                var getStarrer = gitHubClient.Activity.Starring.GetAllForUser(starrer).ContinueWith<object>(tr =>
                {
                    // query faulted
            ...
            Receive<RetryableQuery>(query => query.Query is QueryStarrers, query =>
            {
                ...
                // close over the Sender in an instance variable
                var sender = Sender;
                var getStars = gitHubClient.Activity.Starring.GetAllStargazers(starrers.Owner, starrers.Repo)
